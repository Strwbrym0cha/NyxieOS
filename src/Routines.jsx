...
function RoutineCard({summary,date,lowEnergy,showFull,setShowFull,onToggleStep,onAddStep,onEditStep,onDeleteStep,onSkip,onTryTomorrow}){
  ...
  return <Card>
    <div className="routine-card-head"><div><h2>{routine.name}</h2><div className="chip-row"><span className="status-chip">{routine.category}</span><span className="status-chip">{getRoutineRecurrenceLabel(routine)}</span>{routine.active===false&&<span className="status-chip">Paused</span>}</div></div><span className="routine-progress">{summary.completed} / {summary.total}<small>{summary.percentage}% · {summary.remaining} left</small></span></div>
