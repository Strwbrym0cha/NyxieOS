...
export default function YuuChat({data,setData,setScreen}){
 ...
 return <><header className="yuu-chat-header"><div><small>More · Local planner assistant</small><h1>Yuu-Kun</h1></div><YuuSprite state={history[history.length-1]?.state||'normal'} size="md" /></header><div className="yuu-chat-shell"><div className="yuu-chat-history" ref={historyRef} aria-live="polite">{!history.length&&<div className="yuu-message yuu-message-bubble"><YuuSprite state="normal" size="sm" /><p>{greeting}</p></div>}{history.map(item=><div className={'yuu-message '+(item.role==='user'?'user-message':'yuu-message-bubble')} key={item.id}>{item.role!=='user'&&<YuuSprite state={item.state || "normal"} size="sm" />}<p>{item.text}</p></div>)}{pending&&<div className="yuu-confirm"><p>Ready when you are.</p><div><button className="primary" onClick={confirmPending}>Confirm</button><button className="secondary" onClick={cancelPending}>Cancel</button></div></div>}</div><div className="yuu-quick-prompts" aria-label="Quick prompts">{QUICK_PROMPTS.map(prompt=><button key={prompt} className="quick-prompt" onClick={()=>answer(promptText[prompt])}>{prompt}</button>)}</div><form className="yuu-composer" onSubmit={event=>{event.preventDefault();answer(input);setInput('')}}><input aria-label="Message Yuu-Kun" value={input} onChange={event=>setInput(event.target.value)} placeholder="Ask Yuu-Kun…" autoComplete="off"/><button className="primary" type="submit" aria-label="Send message">Send</button></form></div></>;
}
