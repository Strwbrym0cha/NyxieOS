...
export const YUU_STATES = Object.freeze({
  ...
  money: { label: 'money mode', hair: '#FFD36E', accent: '#C93686' },
  cosplay: { label: 'cosplay mode', hair: '#FFD36E', accent: '#E4489D' }
});
...
export function normalizeYuuState(value) {
  ...
  if (key === 'urgent' || key === 'pushy' || key === 'scolding') return 'nagging';
...
const YUU_ASSETS = Object.freeze({
  normal: ASSET_BASE + 'yuu/yuu-normal.jpg',
  nagging: ASSET_BASE + 'yuu/yuu-nagging.jpg',
  proud: ASSET_BASE + 'yuu/yuu-proud.jpg',
  sleepy: ASSET_BASE + 'yuu/yuu-sleepy.jpg',
  money: ASSET_BASE + 'yuu/yuu-money.jpg',
  cosplay: ASSET_BASE + 'yuu/yuu-cosplay.jpg'
});

function YuuFallback({ normalized, meta }) {
...
export default function YuuSprite({ state = 'normal', size = 'md', className = '' }) {
  ...
  useEffect(() => setImageFailed(false), [normalized]);
  const label = `Yuu-Kun, ${meta.label}`;
  return (
    <span
      ...
      aria-label={label}
    >
      {imageFailed ? <YuuFallback normalized={normalized} meta={meta} /> : (
        <img
          src={YUU_ASSETS[normalized]}
          alt={label}
